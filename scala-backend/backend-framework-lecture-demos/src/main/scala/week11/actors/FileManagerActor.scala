...
import week11.models.{ErrorResponse, SuccessfulResponse}

import scala.util.{Failure, Success, Try}

object FileManagerActor {
  private val filePrefix = "./src/main/resources/s3"
  private val inFilePrefix = "./src/main/resources/in"
  private val outFilePrefix = "./src/main/resources/out"
...
class FileManagerActor(client: AmazonS3, bucketName: String) extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case UploadAllFiles =>
      val mainDirectory: File = new File(outFilePrefix)
      ...
      def uploadDirectoryContents(dir: File): Unit = {
        ...
        for (file <- files) {
          ...
          if (file.isDirectory)
            ...
            uploadFile(client, bucketName, path.toString, outFilePrefix + "/" + path.toString)
    ...
    case DownloadAllFiles =>
      ...
      objects.getObjectSummaries.forEach(objectSummary => downloadFile(client, bucketName, objectSummary.getKey, inFilePrefix + "/" + objectSummary.getKey))
  }
}

//  def downloadFile(inputStream: InputStream, file: File): Unit = {
//    val bis: BufferedInputStream = new BufferedInputStream(inputStream)
//    val bos: BufferedOutputStream = new BufferedOutputStream(new FileOutputStream(file))
//
//    var line: Int = -1
//    while ( {
//      line = bis.read();
